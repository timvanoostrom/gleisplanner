<script lang="ts">
  ...
  $: isActive = $editMode === 'guides' || $editMode === 'shapes';
  ...
  function startOrFinishGuide(event: MouseEvent) {
    if (!event.target.ownerSVGElement || !isActive) {
      return;
    }
  ...
  function moveEndPoint(event: MouseEvent) {
    ...
    if (isActive && newPathPoints.length >= 1) {
  ...
  function addPoint(event: MouseEvent) {
    console.log('addp');
    if (isActive && event.target?.ownerSVGElement) {
      const point = svgCoords(event, guideGroup);
