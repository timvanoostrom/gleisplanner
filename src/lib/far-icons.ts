...
export type IconName = keyof typeof farIcons;
export function icon(name: IconName, size: number = 16) {
  if (!(name in farIcons)) {
    throw new Error(`Icon ${name} not present.`);
  }
  return `&#x${farIcons[name]}`;
}
