...
class train:
    ...
    def get_info(self):
        data = [
            ...
            self.dcc & 0xFF,
            self.stufen,
            (self.forward << 7) + (self.speed & 0x7F),
            (self.doppel << 6)
            + (self.Smart << 5)
            + (self.F[0] << 4)
            + (self.F[4] << 3)
...
class Z21_message:
        ...
        if header is 0x10:
            ...
            if XHeader == 0x21:  # LAN_X_GET_VERSION
                ...
                if message[5] & 0xF0 == 0x10:
                    ...
                    # try:
                    if dcc in trains:
                        trains[dcc].set_speed(
                            message[5] & 0x3, message[8] & 0x7F, message[8] >> 7
                        )
                        print(
                    ...
                    if dcc in trains:
                        trains[dcc].set_func(
                            (message[8] & 0xC0) >> 6, message[8] & 0x3F
                        )
                        # Z21.server.write(trains[dcc].get_info(), addr)
                    else:
                        trains[dcc] = train(dcc)
            ...
            self.send(
                addr,
                self.LAN_GET_BROADCASTFLAGS,
                flags=[c.flags for c in Z21.clients if c.addr == addr][0],
            )
    ...
    def SYSTEMSTATE_DATACHANGED(self, Z21=None):
        ...
        data += (
            Z21.emergency_stop * 0x01
            + Z21.track_power_off * 0x02
            + Z21.short * 0x04
            + Z21.prog_mode * 0x20
    ...
    def LAN_GET_LOCOMODE(self, data1=0, data2=0):
        return (
            b"\x07\x00\x60\x00"
            + data1.to_bytes(1, "little")
            + data2.to_bytes(1, "little")
            + b"\x00"
            + (data1 ^ data2).to_bytes(1, "little")
        )
